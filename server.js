...
app.use(cors()); // allow calls from GPT Action builder, etc.
...
async function getTopics(user_id) {
  if (!user_id) return [];
  const topics = await redis.get(key(user_id));
  return Array.isArray(topics) ? topics : (topics ? JSON.parse(topics) : []);
}
